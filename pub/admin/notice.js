...
function show_notice(notice){
	...
	getUserInfo(notice.user).then(userInfo => {
		...
		div.innerHTML += `<div class='notice_info'><a class='admin_name' href="/profile?user_id=${notice.user}">${userInfo.displayname}(@${userInfo.username})</a> - <a class='post_time'>${readableDate(notice.time)}</a></div>`;
	})
	if (notice.isShowing){
